...
def evaluate_default(dataset, results, keyword, logger):
    dataset.evaluate(results, keyword, logger, topk=(1, 5))
    ...
    if rank == 0:
        if args.data_mode == 'default':
            for name, val in outputs.items():
                evaluate_default(dataset, torch.from_numpy(val), name, logger)
        elif args.data_mode == 'coco':
            evaluate_default(dataset, outputs, None, logger)
