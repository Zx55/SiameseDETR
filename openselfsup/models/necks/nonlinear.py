...
class NonLinearNeckV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(hid_channels, out_channels))
        if sync_bn:
            self.mlp = nn.SyncBatchNorm.convert_sync_batchnorm(self.mlp)
...
class NonLinearNeckV3(nn.Module):
    """The non-linear neck in simsiam: fc-bn-relu-fc-bn-relu-fc-bn.
    """
    ...
    def forward(self, x):
        ...
        x = x[0]
        if self.with_avg_pool:
            if isinstance(x, (list, tuple)):  # multi scale features
                x = x[:3] 
                x = torch.cat([self.avgpool(x_) for x_ in x], dim=1)
            else:
                x = self.avgpool(x)
